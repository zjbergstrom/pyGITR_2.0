...
def loadGITRBGP(inFile="profiles.nc",gridR="r",gridZ="z",variable="ne"):
    ...
    var = np.array(ncFile.variables[variable])

    return nR,nZ,r,z,variable


def plotGITRBGP(filename="profiles_created",rangeR=[1.37875,1.59125], rangeZ=[-1.25,-1.0375], param='te', plot=0):


    nR,nZ,r,z,var = loadGITRBGP(filename,r,z,param)

    r,z = np.meshgrid(r,z)

    if param == "te" or param == "ti": cmap="Greens"
    if param == "ne" or param == "ni": cmap="Reds"
    if param == "br" or param == "bt" or param == "bz": cmap="Purples"
    else: cmap="Blues"

    c=plt.scatter(r, z, vmax=np.max(var), vmin=np.min(var),  c=var, cmap=cmap, label=param, marker='s')
    plt.colorbar(c)
    plt.xlim(rangeR)
    plt.ylim(rangeZ)
    # plt.axis('scaled')
